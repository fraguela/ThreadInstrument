...
///
/// \file     pfor_simpl.cpp
/// \brief    Same as pfor.cpp but relying on THREADINSTRUMENT_PROF
...
struct ParallelStuff {
  ...
  void operator()(int begin, int end) const  {
    ...
    THREADINSTRUMENT_PROF("SEQ_ACT",
                          mx((float *)c, (float *)a, (float *)b, 80);
    );
      
    THREADINSTRUMENT_PROF("MISC_ACT",
...
{ std::vector<std::thread> v;
  ...
  for(unsigned i = 0; i < ThreadInstrument::nThreadsWithActivity(); ++i) {
    ...
    // Test dump to file
    ThreadInstrument::dumpActivity(activity, nullptr, "/tmp/pfor_simpl2_activities.txt");
  }
